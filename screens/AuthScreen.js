...
export default function AuthScreen({ navigation }) {
  ...
  const initializeAuth = async () => {
    try {
      // iOS needs delay for SecureStore to be ready
      if (Platform.OS === 'ios') {
  ...
  const authenticateWithPin = async () => {
    // Dismiss keyboard on iOS to prevent crash
    if (Platform.OS === 'ios') {
      Keyboard.dismiss();
    }
    ...
    try {
      // Add delay for iOS SecureStore to be ready
      if (Platform.OS === 'ios') {
  ...
  const setupPin = async () => {
    ...
    try {
      ...
      Alert.alert('Setup Error', 'Failed to set up PIN. Please try again.');
      setPin('');
      setConfirmPin('');
    }
  };
